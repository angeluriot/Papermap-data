...
MIN_SELF_CITATION = 0.05 # The minimum self-citation
MAX_SELF_CITATION = 0.75 # The maximum self-citation
...
def compute_scores(metric_values: dict[str, int | float], metric: str) -> dict[str, float]:
	...
	if metric == 'self':
		return {
			id: exp_curve(1.0 - ratio(value, MIN_SELF_CITATION, MAX_SELF_CITATION), metric)
...
def generate_scores() -> None:
	...
	if os.path.exists(os.path.join('journals.csv')):
		os.remove(os.path.join('journals.csv'))
	...
	for journal in journals_list:
		...
		for metric in METRICS:
			result[f'metric_{metric}'] = journal['metrics'].get(metric)

		for metric in METRICS:
			result[f'score_{metric}'] = journal['scores'].get(metric)

		results.append(result)

	with open(os.path.join('journals.csv'), 'w', newline='', encoding='utf-8') as file:
		pd.DataFrame(results).to_csv(file, index=False, lineterminator='\n', encoding='utf-8')

	# === Print === #

	print('Top 10 journals:\n')

	for i, journal in enumerate(results[:10]):
		print(f'{i + 1}.{"  " if i != 9 else " "}{journal["title"]} ({journal["score"]:.3f})')
