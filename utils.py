...
SCOPE_GROUP_TO_ID = {name.lower(): code for code, name in ID_TO_SCOPE_GROUP.items()}
SCOPE_TO_ID = {name.lower(): code for code, name in ID_TO_SCOPE.items()}

JOURNAL_TYPES = {
	'journal': 'Journal',
	'book series': 'Book Series',
	'conference and proceedings': 'Conference and Proceedings',
	'conference proceeding': 'Conference and Proceedings',
...
def clean_text(s: str) -> str:
	return ''.join([c for c in s.strip().replace('&amp;', '&').replace('&', 'and').lower() if c.isalpha() or c.isnumeric()])

def clean_issn(s: str) -> str:
	return ''.join([c for c in s.strip() if c.isalpha() or c.isnumeric()]).upper()

def clean_scope(s: str) -> str:
	return s.lower().replace('(q1)', '').replace('(q2)', '').replace('(q3)', '').replace('(q4)', '').replace('(miscellaneous)', '').strip()

def print_values(l: list[int], inline = True, description = '') -> None:
	nbs = {}
	...
	for k in sorted(nbs.keys()):
		print(f'{k:,}: {nbs[k]:,}', end = '   ' if inline else '\n')
