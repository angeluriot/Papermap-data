...
This repository contains the code to generate the **[Papermap](https://github.com/angeluriot/Papermap)** journals database and compute their `Journal Score`.
...
The initial journals data is retrieved from the **[OpenAlex](https://openalex.org/)** database to create a list of 209,819 journals available in the **[/data/journals.json](/data/journals.json)** file.
...
All the metrics are stored in the **[/data/journals.json](/data/journals.json)** file, run the **[/data.ipynb](/data.ipynb)** notebook to update it.

<br/>

# 🧮 Score

The naive way to calculate a `Journal Score` would be to take the average of all these metrics, but this would be biased towards the ones with the highest values. A simple solution would be to normalize them before averaging (between 0 and 1 for example), but their distributions are still very different:

<p align="center">
	<img src="resources/misc/metrics.png" width="750">
</p>
...
<p align="center">
	<img src="resources/misc/metric_ranks.png" width="750">
</p>
...
<p align="center">
	<img src="resources/misc/metrics_exp.png" width="750">
</p>
...
<p align="center">
	<img src="resources/misc/metric_scores.png" width="750">
</p>

A metric that was ignored until now is the **[Self-citation rate](https://www.journalindicators.com/indicators)**, this metric is special because here a high value is clearly a bad thing (meaning it should be inverted) but at the same time a journal with 1% is probably better than a journal with 0%. Therefore, all the values above 5% are inverted and the others are set to 1 (the transformation is also applied to emphasize high self-citation rates):

<p align="center">
	<img src="resources/misc/self.png" width="750">
</p>

When averaging the metrics to create the final score, an important point is missing data, some journals have no value for some metrics. To avoid cases where a journal gets a really high score based on a single metric, a penalty (0.1) is added to the denominator of the average for each missing metric.

The distribution of the final scores is a bit too low for the first journals and a bit too high for the last ones compared to the metrics. To correct this, the same steps as described above (ranking + transformation) are also implemented, but here the result is combined with the initial score instead of replacing it:

<p align="center">
	<img src="resources/misc/scores.png" width="750">
</p>

The code that implements all these calculations is available in the **[/scores.py](/scores.py)** file.

The final scores are stored in the **[/journals.json](/journals.json)** file, run the **[/scores.ipynb](/scores.ipynb)** notebook to update it.

<br/>

# 🏆 Results

Here are the top 10 journals based on their `Journal Score`:
