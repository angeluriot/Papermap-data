...
def filter_pairs(
	...
	for r, j in pairs:
		...
		if row['scopus_id'] == journal['scopus_id']:
			points += 1
		...
		if row['title'] == journal['title']:
			points += 1

		points *= 10

		if over_clean_text(row['title']) == over_clean_text(journal['title']):
			points += 1

		points *= 10
		row_titles = [over_clean_text(title) for title in ([row['title']] + row['titles'])]
		journal_titles = [over_clean_text(title) for title in ([journal['title']] + journal['titles'])]
		points += len(set(row_titles).intersection(set(journal_titles)))
		points *= 10
		...
		if publisher is not None:
			row_publishers = [over_clean_text(p) for p in row['publisher_titles']]
			journal_publishers = [over_clean_text(p) for p in ([publisher['title']] + publisher['titles'])]
			points += len(set(row_publishers).intersection(set(journal_publishers)))
		...
		points *= 10
		points += len(set(journal['fields']).intersection(set(row['fields'])))
		points *= 1_000

		if row['last_year'] is not None:
			points += row['last_year'] - 1700
